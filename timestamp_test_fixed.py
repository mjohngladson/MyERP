...
BACKEND_URL = "https://retail-nexus-18.preview.emergentagent.com"
                    ...
                    if send_response.get("success"):
                        ...
                        async with self.session.get(f"{self.base_url}/api/sales/credit-notes/{credit_note_id}") as get_response:
                            if get_response.status == 200:
                                ...
                                if new_last_sent_at:
                                    try:
                                        new_timestamp = parse_timestamp(new_last_sent_at)
                                        old_timestamp_parsed = parse_timestamp(old_last_sent_at)
                                        
                                        if new_timestamp and old_timestamp_parsed:
                                            ...
                                            old_time_diff = (send_time_after - old_timestamp_parsed).total_seconds()
                                            
                                            print(f"   📊 Time Analysis:")
                                            print(f"      Old timestamp: {old_last_sent_at} (diff: {old_time_diff:.1f}s ago)")
                                            print(f"      New timestamp: {new_last_sent_at} (diff: {time_diff_seconds:.1f}s ago)")
                                            print(f"      Send time: {send_time_after.isoformat()}")
                                            
                                            if abs(time_diff_seconds) < 60:  # Within 1 minute of send time
                                                self.log_test("Verify Timestamp Update", True, f"✅ TIMESTAMP CORRECTLY UPDATED: New last_sent_at is current (within {abs(time_diff_seconds):.1f}s)")
                                                ...
                                                self.log_test("Verify Timestamp Update", False, f"❌ TIMESTAMP NOT UPDATED: New timestamp is {abs(time_diff_seconds):.1f}s from send time (should be <60s). This is the reported bug!")
                                                result = False
                                        else:
                                            self.log_test("Verify Timestamp Update", False, "Failed to parse timestamps")
                                            result = False
                                    except Exception as e:
                                        self.log_test("Verify Timestamp Update", False, f"Error processing timestamps: {str(e)}")
                                        result = False
                                else:
