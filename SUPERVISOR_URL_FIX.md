...
## Problem
The frontend application was pointing to Railway backend URL (`https://myerp-production.up.railway.app`) instead of Preview backend URL (`https://erp-nextgen.preview.emergentagent.com`), despite the `.env` file being correctly configured.
...
[program:frontend]
command=yarn start
environment=HOST="0.0.0.0",PORT="3000",REACT_APP_BACKEND_URL="https://erp-nextgen.preview.emergentagent.com",DANGEROUSLY_DISABLE_HOST_CHECK="true"
```
...
- Process environment check: `ps auxe | grep "react-scripts" | grep REACT_APP_BACKEND_URL`
  - Result: `REACT_APP_BACKEND_URL=https://erp-nextgen.preview.emergentagent.com` ✅

- Backend API test: `curl https://erp-nextgen.preview.emergentagent.com/api/financial/accounts`
