...
import QuotationsOldList from './components/QuotationsList';
...
const MainApp = () => {
  ...
  const renderActiveComponent = () => {
    switch (activeView) {
      ...
      case 'quotations':
        return <QuotationsList onBack={() => setActiveView('dashboard')} onViewQuotation={(q)=>{ setSelectedItem(q); setActiveView('quotation-view'); }} onEditQuotation={(q)=>{ setSelectedItem(q); setActiveView('quotation-edit'); }} onCreateQuotation={()=> setActiveView('quotation-form')} />;
      case 'quotation-form':
        return <QuotationForm onBack={() => setActiveView('quotations')} onSave={()=> setActiveView('quotations')} />;
      case 'quotation-edit':
        return <QuotationForm quotationId={selectedItem?.id} onBack={() => setActiveView('quotations')} onSave={()=> setActiveView('quotations')} />;
      case 'quotation-view':
        return <QuotationView quotationId={selectedItem?.id} initialQuotation={selectedItem} onBack={() => setActiveView('quotations')} />;
      // Buying
  ...
  if (loading) { return (<div className="min-h-screen bg-gray-50 flex items-center justify-center"><div className="text-center"><div className="w-16 h-16 bg-blue-600 rounded-xl flex items-center justify-center mx-auto mb-4"><div className="w-8 h-8 border-4 border-white border-t-transparent rounded-full animate-spin"></div></div><p className="text-gray-600">Loading...</p></div></div>); }
