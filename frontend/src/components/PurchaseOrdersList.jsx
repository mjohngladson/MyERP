...
const PurchaseOrdersList = ({ onBack, onViewOrder, onEditOrder, onCreateOrder }) => {
  ...
  const filteredOrders = orders.filter(o => {
    const matchesSearch = searchTerm === '' || `${o.order_number||''}`.toLowerCase().includes(searchTerm.toLowerCase()) || `${o.supplier_name||''}`.toLowerCase().includes(searchTerm.toLowerCase());
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex flex-col sm:flex-row gap-4 flex-1">
            <div className="relative flex-1 max-w-md">
              ...
              <input type="text" placeholder="Search purchase orders..." value={searchTerm} onChange={(e)=>setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500" />
            </div>
