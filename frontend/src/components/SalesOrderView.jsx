...
const SalesOrderView = ({ orderId, initialOrder, onBack }) => {
    ...
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center space-x-4">
          ...
          <h1 className="text-2xl font-semibold text-gray-900">Sales Order {order?.order_number || ''}</h1>
        </div>
          ...
          <>
            <div className="flex items-start justify-between">
              ...
              <div className="text-right text-sm text-gray-600">
                <div>Order No: <span className="font-medium text-gray-900">{order.order_number}</span></div>
                <div>Date: {formatDate(order.order_date || order.created_at)}</div>
                <div>Delivery: {formatDate(order.delivery_date)}</div>
            ...
            <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <div className="text-sm text-gray-600">{order.shipping_address}</div>
              </div>
            </div>
            ...
            <div className="mt-6 flex justify-end"><div className="w-80 space-y-2 text-sm"><div className="flex justify-between"><span className="text-gray-600">Total</span><span className="font-medium">{formatCurrency(order.total_amount)}</span></div></div></div>
          </>
        )}
      </div>
    </div>
