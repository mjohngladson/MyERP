  ...
  const submitSend = async () => {
    ...
    try {
      const res = await fetch(`${api.getBaseUrl()}/api/invoices/${sendTarget.id}/send`, {
        ...
        body: JSON.stringify({ email: sendEmail, phone: sendPhone })
      });
      const data = await res.json();
      if (res.ok && data.success) {
        alert('Invoice sent successfully via email' + (sendPhone ? ' (SMS pending provider setup)' : ''));
        setSendOpen(false);
      } else {
                  ...
                  <tr key={invoice.id || invoice.invoice_number} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <div className="flex items-center space-x-2">
                        ...
                        <button onClick={() => { if (confirm(`Delete invoice ${invoice.invoice_number}?`)) {/* implement delete elsewhere if needed */} }} className="text-red-600 hover:text-red-900 p-1" title="Delete Invoice"><Trash2 className="h-4 w-4" /></button>
                      </div>
                    </td>
                  </tr>
                ))}
          ...
          <div className="bg-white rounded-lg shadow-lg w-full max-w-md p-6">
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">SMS sending will be enabled after SMS provider setup.</p>
              </div>
            </div>
