...
const CreditNoteForm = ({ creditNoteId, onBack, onSave }) => {
    ...
    const loadMasterData = async () => {
      try {
        const [itemsRes, customersRes, invoicesRes] = await Promise.all([
          ...
          api.get('/invoices/', { params: { limit: 200 } })  // Added trailing slash to avoid FastAPI redirect
        ]);
      ...
      const load = async () => {
        try {
          ...
          setForm({
            ...
            credit_note_date: data.credit_note_date ? data.credit_note_date.split('T')[0] : new Date().toISOString().split('T')[0],
          ...
          console.error('Failed to load credit note:', e);
        } finally {
          setLoading(false);
        }
      };
  ...
  const save = async () => {
    ...
    try {
      ...
      if (creditNoteId) {
        await api.put(`/sales/credit-notes/${creditNoteId}`, payload);
      } else {
        await api.post('/sales/credit-notes', payload);
      }
      
      onSave && onSave();
    } catch (e) {
      alert(e?.response?.data?.detail || 'Failed to save credit note');
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center">
          ...
          <h1 className="text-2xl font-bold text-gray-800">
            {creditNoteId ? 'Edit Credit Note' : 'Create Credit Note'}
          </h1>
        </div>
        <button 
          onClick={save} 
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <AutocompleteSearch
                  options={customers.map(customer => ({
                    ...customer,
                    subtitle: customer.email || customer.phone || customer.mobile
                  }))}
                  ...
                  onSelect={customer => {
                    if (customer && typeof customer === 'object') {
                      ...
                      updateForm('customer_name', customer || '');
                    }
                  }}
                  ...
                  allowCustom={true}
                  customPlaceholder="Or enter custom customer name"
                  renderOption={(customer, index, isHighlighted) => (
                    <div
                    ...
                    >
                      ...
                      {customer.subtitle && (
                        <div className="text-xs text-gray-500">{customer.subtitle}</div>
                      )}
                    </div>
                  )}
                />
              ...
              <div>
                ...
                <input
                  ...
                  onChange={e => updateForm('credit_note_date', e.target.value)}
                  className="w-full px-3 py-2 border rounded-md focus:ring-2 focus:ring-blue-500"
                />
              </div>
              <div>
